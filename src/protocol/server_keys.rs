...
pub struct ServerKeysResponse {
    ...
    pub old_verify_keys: BTreeMap<String, VerifyKey>,
}
...
pub fn make_server_keys(
        ...
        verify_keys: verify_keys
            ...
            .map(|(name, (pubkey, _))| {
                (
                    name.clone(),
                    VerifyKey {
                        key: base64::encode_config(pubkey.as_ref(), base64::STANDARD_NO_PAD),
                    },
                )
            })
            .collect(),
    ...
    let mut s = Signed::wrap(server_keys).expect("server keys to be valid json");

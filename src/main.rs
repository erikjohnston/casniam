...
mod tests {
    ...
    struct TestEventStore {
        ...
        state_map: HashMap<String, StateMap<String>>,
    }
    ...
    impl EventStore for TestEventStore {
        fn missing_events<'a, I: IntoIterator<Item = &'a str>>(
            ...
            let missing: Vec<_> = event_ids
                .into_iter()
                .filter(|e| !self.event_map.contains_key(e as &str))
                .map(|e| e.to_string())
                .collect();

            FutureObj::new(Box::new(future::ok(missing)))
        }

        fn get_events(
            &self,
            event_ids: &[&str],
        ) -> FutureObj<Result<Vec<Event>, ()>> {
            let events: Vec<_> = event_ids
                .into_iter()
                .filter_map(|e| self.event_map.get(e as &str))
                .cloned()
                .collect();

            FutureObj::new(Box::new(future::ok(events)))
        }

        fn get_state_for(
            &self,
            event_ids: &[&str],
        ) -> FutureObj<Result<Option<StateMap<String>>, ()>> {
            assert_eq!(event_ids.len(), 0);

            let event_id = event_ids[0];

            let state_map = self.state_map.get(event_id).cloned();

            FutureObj::new(Box::new(future::ok(state_map)))
        }
    }
